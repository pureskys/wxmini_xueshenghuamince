// pages/jiaoshi_zhuce/jiaoshi_zhuce.js
Page({
    ...
    // 确认注册按钮
    zhuce() {
        if (this.data.username.length > 0 && this.data.password.length > 0 && this.data.name.length > 0 && this.data.tele.length > 0) {
    ...
    get_fangfa() {
        ...
        let cloudPath = "Root" + "/" + this.data.name + "." + extname
        wx.cloud.uploadFile({
                cloudPath: cloudPath,
    ...
    // 提交信息
    getdata() {
        wx.cloud.database().collection("Root").add({
                data: {
                    user: {
                        age: this.data.age,
                    ...
                    my_class: this.data.class1,
                    identity:this.data.value3,
                    yanzheng:false
                }
            })
